...
static void nrf_get_state(nrfHeader inNRF, NRF_STATE_CONFIG* pnrf_state){
	...
	RX_PW_P0 rx_pw_p0;-
	 */
	NRF24L01_get_config(inNRF,(uint8_t*)&pnrf_state->config_reg);
...
void task_nrf24l01(void){
   ...
   /*----------------START RECEIVER---------------------*/
	...
	NRF24L01_clear_interrupt(NRF2, MASK_RX_DR); // Clear status NRF
	//Set Rx mode
	...
	while(status_reg.RX_DR==0){
		NRF24L01_get_status(NRF2,(uint8_t*)&status_reg);
		NRF24L01_get_status(NRF1,(uint8_t*)&status_reg1);
	}; //wait interrupt
	NRF24L01_clear_interrupt(NRF2,MASK_RX_DR);
	NRF24L01_get_status(NRF2,(uint8_t*)&status_reg);
